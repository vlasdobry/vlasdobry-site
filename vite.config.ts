import path from 'path';
import { defineConfig } from 'vite';
import react from '@vitejs/plugin-react';
import tailwindcss from '@tailwindcss/vite';

export default defineConfig({
    server: {
      port: 3000,
      host: '0.0.0.0',
      proxy: {
        '/api/health-score': {
          target: 'https://health-score-proxy.vlasdobry.workers.dev',
          changeOrigin: true,
          headers: {
            Origin: 'https://vlasdobry.ru',
          },
          rewrite: () => '/',
        },
        '/api/compliance': {
          target: 'https://health-score-proxy.vlasdobry.workers.dev',
          changeOrigin: true,
          headers: {
            Origin: 'https://vlasdobry.ru',
          },
        },
      },
    },
    plugins: [
      react(),
      tailwindcss(),
      {
        name: 'rewrite-routes',
        configureServer(server) {
          const routes = [
            // Blog posts (must be before blog list)
            { prefix: '/en/blog/', file: '/blog-post.html', isPost: true },
            { prefix: '/blog/', file: '/blog-post.html', isPost: true },
            // Blog list
            { prefix: '/en/blog', file: '/blog-en.html' },
            { prefix: '/blog', file: '/blog.html' },
            { prefix: '/en/services/ppc', file: '/ppc-en.html' },
            { prefix: '/en/services/seo', file: '/seo-en.html' },
            { prefix: '/en/services/geo', file: '/geo-en.html' },
            { prefix: '/services/ppc', file: '/ppc.html' },
            { prefix: '/services/seo', file: '/seo.html' },
            { prefix: '/services/geo', file: '/geo.html' },
            { prefix: '/en/168-fz', file: '/168-fz-en.html' },
            { prefix: '/168-fz', file: '/168-fz.html' },
            { prefix: '/en/extension/privacy', file: '/extension-privacy-en.html' },
            { prefix: '/extension/privacy', file: '/extension-privacy.html' },
            { prefix: '/en/case-dna-labs', file: '/case-dna-labs-en.html' },
            { prefix: '/case-dna-labs', file: '/case-dna-labs.html' },
            { prefix: '/en/for-hotels', file: '/for-hotels-en.html' },
            { prefix: '/en/for-labs', file: '/for-labs-en.html' },
            { prefix: '/en/for-spa', file: '/for-spa-en.html' },
            { prefix: '/en/projects', file: '/projects-en.html' },
            { prefix: '/for-hotels', file: '/for-hotels.html' },
            { prefix: '/for-labs', file: '/for-labs.html' },
            { prefix: '/for-spa', file: '/for-spa.html' },
            { prefix: '/projects', file: '/projects.html' },
            { prefix: '/en/terms', file: '/terms-en.html' },
            { prefix: '/en/privacy', file: '/privacy-en.html' },
            { prefix: '/en/about', file: '/about-en.html' },
            { prefix: '/terms', file: '/terms.html' },
            { prefix: '/privacy', file: '/privacy.html' },
            { prefix: '/about', file: '/about.html' },
            { prefix: '/en', file: '/en.html' },
          ];

          server.middlewares.use((req, _res, next) => {
            const url = req.url || '';
            if (!url.includes('.')) {
              const route = routes.find(r => {
                if (r.isPost) {
                  // Blog post: /blog/[slug]/ but not /blog/ itself
                  const match = url.match(new RegExp(`^${r.prefix}[^/]+/?$`));
                  return match !== null;
                }
                return url.startsWith(r.prefix);
              });
              if (route) req.url = route.file;
            }
            next();
          });
        },
      },
    ],
    resolve: {
      alias: {
        '@': path.resolve(__dirname, '.'),
      }
    },
    build: {
      rollupOptions: {
        input: {
          main: path.resolve(__dirname, 'index.html'),
          en: path.resolve(__dirname, 'en.html'),
          'for-hotels': path.resolve(__dirname, 'for-hotels.html'),
          'for-hotels-en': path.resolve(__dirname, 'for-hotels-en.html'),
          'for-labs': path.resolve(__dirname, 'for-labs.html'),
          'for-labs-en': path.resolve(__dirname, 'for-labs-en.html'),
          'for-spa': path.resolve(__dirname, 'for-spa.html'),
          'for-spa-en': path.resolve(__dirname, 'for-spa-en.html'),
          'projects': path.resolve(__dirname, 'projects.html'),
          'projects-en': path.resolve(__dirname, 'projects-en.html'),
          'seo': path.resolve(__dirname, 'seo.html'),
          'seo-en': path.resolve(__dirname, 'seo-en.html'),
          'geo': path.resolve(__dirname, 'geo.html'),
          'geo-en': path.resolve(__dirname, 'geo-en.html'),
          'ppc': path.resolve(__dirname, 'ppc.html'),
          'ppc-en': path.resolve(__dirname, 'ppc-en.html'),
          'blog': path.resolve(__dirname, 'blog.html'),
          'blog-en': path.resolve(__dirname, 'blog-en.html'),
          'blog-post': path.resolve(__dirname, 'blog-post.html'),
          '168-fz': path.resolve(__dirname, '168-fz.html'),
          '168-fz-en': path.resolve(__dirname, '168-fz-en.html'),
          'extension-privacy': path.resolve(__dirname, 'extension-privacy.html'),
          'extension-privacy-en': path.resolve(__dirname, 'extension-privacy-en.html'),
          'case-dna-labs': path.resolve(__dirname, 'case-dna-labs.html'),
          'case-dna-labs-en': path.resolve(__dirname, 'case-dna-labs-en.html'),
          'about': path.resolve(__dirname, 'about.html'),
          'privacy': path.resolve(__dirname, 'privacy.html'),
          'terms': path.resolve(__dirname, 'terms.html'),
          'about-en': path.resolve(__dirname, 'about-en.html'),
          'privacy-en': path.resolve(__dirname, 'privacy-en.html'),
          'terms-en': path.resolve(__dirname, 'terms-en.html'),
        },
      },
    },
});
